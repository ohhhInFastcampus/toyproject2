...
const Calendar = () => {
  ...
  const handleEditModal = (clickedEvent: any) => {
    const event: ScheduleType = {
      userId: "",
  ...
  const handleEditEvent = (formData: ScheduleType) => {
    const updatedEvents = events.map(event => event.id === formData.id ? formData : event);
    setEvents(updatedEvents);
    setShowEditModal(false);
  }
  ...
  const handleFormSubmit = (formData: ScheduleType) => {
    const event: ScheduleType = {
      ...formData,
      id: `${new Date().getTime()}`
    };
